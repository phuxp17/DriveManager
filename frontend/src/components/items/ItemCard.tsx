import React from 'react';
import {
  Archive,
  ArchiveRestore,
  CheckCircle2,
  ExternalLink,
  FolderPlus,
  MoreVertical,
  Share2,
  Star,
  Trash2,
  Undo2,
} from 'lucide-react';
import { ItemEntry } from '../../api/types';
import { formatDate } from '../../utils/dateUtils';
import { DropdownMenu, DropdownMenuItem } from '../common/DropdownMenu';
import { ItemTypeIcon } from './ItemTypeIcon';
import { ItemActionHandlers } from './ItemTableRow';
import styles from './ItemCard.module.css';

export interface ItemCardProps {
  item: ItemEntry;
  currentUserId?: string;
  actions: ItemActionHandlers;
}

export const ItemCard: React.FC<ItemCardProps> = ({ item, currentUserId, actions }) => {
  const isOwner = !currentUserId || currentUserId === item.ownerId;
  const isFavorited = !!item.favoritedAt;
  const isArchived = !!item.archivedAt;
  const isTrashed = !!item.deletedAt;
  const isReviewed = !!item.reviewedAt;

  const menuItems: (DropdownMenuItem | 'separator')[] = [
    {
      label: item.type === 'LINK' ? 'Mở liên kết' : 'Tải / Mở tệp',
      icon: <ExternalLink size={16} />,
      onClick: () => actions.onOpen(item),
    },
    {
      label: 'Xem chi tiết',
      onClick: () => actions.onViewDetails(item),
    },
    {
      label: isFavorited ? 'Bỏ yêu thích' : 'Yêu thích',
      icon: <Star size={16} />,
      onClick: () => actions.onToggleFavorite(item),
    },
  ];

  if (isOwner && !isTrashed) {
    menuItems.push('separator');

    if (actions.onToggleReview) {
      menuItems.push({
        label: isReviewed ? 'Đưa vào Inbox' : 'Đã duyệt',
        icon: <CheckCircle2 size={16} />,
        onClick: () => actions.onToggleReview!(item),
      });
    }

    if (actions.onManageMemberships) {
      menuItems.push({
        label: 'Bộ sưu tập & Thẻ',
        icon: <FolderPlus size={16} />,
        onClick: () => actions.onManageMemberships!(item),
      });
    }

    if (actions.onShare) {
      menuItems.push({
        label: 'Chia sẻ (VIEW)',
        icon: <Share2 size={16} />,
        onClick: () => actions.onShare!(item),
      });
    }

    if (actions.onToggleArchive) {
      menuItems.push({
        label: isArchived ? 'Bỏ lưu trữ' : 'Lưu trữ',
        icon: isArchived ? <ArchiveRestore size={16} /> : <Archive size={16} />,
        onClick: () => actions.onToggleArchive!(item),
      });
    }

    if (actions.onTrash) {
      menuItems.push({
        label: 'Chuyển vào Thùng rác',
        icon: <Trash2 size={16} />,
        danger: true,
        onClick: () => actions.onTrash!(item),
      });
    }
  }

  if (isOwner && isTrashed) {
    menuItems.push('separator');
    if (actions.onRestore) {
      menuItems.push({
        label: 'Khôi phục',
        icon: <Undo2 size={16} />,
        onClick: () => actions.onRestore!(item),
      });
    }
    if (actions.onPurge) {
      menuItems.push({
        label: 'Xóa vĩnh viễn',
        icon: <Trash2 size={16} />,
        danger: true,
        onClick: () => actions.onPurge!(item),
      });
    }
  }

  return (
    <div className={styles.card}>
      <div>
        <div className={styles.header}>
          <div className={styles.titleArea}>
            <ItemTypeIcon type={item.type} size={22} />
            <div style={{ minWidth: 0, flex: 1 }}>
              <button
                type="button"
                className={styles.name}
                title={item.name}
                onClick={() => actions.onViewDetails(item)}
              >
                {item.name}
              </button>
              <div className={styles.subInfo} title={item.url || item.description || ''}>
                {item.domain || item.description || item.type}
              </div>
            </div>
          </div>

          <div style={{ display: 'flex', alignItems: 'center' }}>
            <button
              className={`${styles.starBtn} ${isFavorited ? styles.isFavorited : ''}`}
              onClick={() => actions.onToggleFavorite(item)}
              aria-label={isFavorited ? 'Bỏ đánh dấu yêu thích' : 'Đánh dấu yêu thích'}
            >
              <Star size={16} fill={isFavorited ? 'currentColor' : 'none'} />
            </button>

            <DropdownMenu
              trigger={
                <button
                  aria-label="Tùy chọn khác"
                  style={{
                    padding: '4px',
                    color: 'var(--color-text-muted)',
                    borderRadius: 'var(--radius-sm)',
                    cursor: 'pointer',
                  }}
                >
                  <MoreVertical size={16} />
                </button>
              }
              items={menuItems}
            />
          </div>
        </div>

        {item.description && (
          <p className={styles.description} title={item.description}>
            {item.description}
          </p>
        )}
      </div>

      <div className={styles.footer}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span className={styles.badge}>{item.type}</span>
          {item.reviewedAt ? (
            <span
              className={styles.badge}
              style={{ color: 'var(--color-success)', backgroundColor: 'var(--color-success-subtle)' }}
            >
              Đã duyệt
            </span>
          ) : (
            <span
              className={styles.badge}
              style={{ color: 'var(--color-warning)', backgroundColor: 'var(--color-warning-subtle)' }}
            >
              Inbox
            </span>
          )}
        </div>

        <span>{formatDate(item.createdAt)}</span>
      </div>
    </div>
  );
};
