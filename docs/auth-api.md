...
3. POST `/api/v1/auth/register` with `{email,password,displayName}` creates a user (201) but does not log in.
4. POST `/api/v1/auth/login` with `{email,password}` returns the public user, rotates the session ID and clears the prior CSRF token.
5. Fetch `/csrf` again after login before the next mutation. GET `/api/v1/auth/me` reads the authenticated identity.
6. POST `/api/v1/auth/logout` with the current CSRF token invalidates the database session. Clear user/query caches and fetch a new `/csrf` token before logging in again.
